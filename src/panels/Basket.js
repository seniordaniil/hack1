...
const useQuery = () => qs.parse(window.location.search.slice(1));
...
const Basket = ({
  ...
  const [faster, setFaster] = useState(
    query.faster ? Boolean(parseInt(query.faster)) : true
  );
  const [time, setTime] = useState(query.time || '');
  const [selfService, setSelfService] = useState(
    query.self_service ? Boolean(parseInt(query.self_service)) : false
  );
    ...
    <div className="Place">
      ...
      <div className="Place__products-wrapper">
        ...
        <Link
          ...
          to={`/place/${areaId}/${itemId}?faster=${Number(
            faster
          )}&time=${time}&self_service=${Number(selfService)}`}
        >
          Изменить
        </Link>
      </div>
